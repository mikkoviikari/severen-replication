...
preserve
	...
	* All shares 
	ivreghdfe 	DOmega_w_pmlall_noT (Dlwage=s_ind_? s_ind_??) [aw=wpop90], a(one) robust 
	...
	* liml
	ivreghdfe 	DOmega_w_pmlall_noT (Dlwage=s_ind_? s_ind_??) [aw=wpop90], a(one) robust liml
	...
	* MBTSLS
	btsls, z(s_ind_? s_ind_??) x(Dlwage) y(DOmega_w_pmlall_noT) ktype("mbtsls") weight_var(wpop90)
	*return list
	local est_mbtsls_b = string(r(beta), "%9.3f")
	
	*HFUL
	overid_chao, z(s_ind_? s_ind_??) x(Dlwage) y(DOmega_w_pmlall_noT) weight_var(wpop90)
	*return list
...
preserve
	...
	bootstrap t=r(beta), rep(400): btsls, z(s_ind_? s_ind_??) x(Dlwage) y(DOmega_w_pmlall_noT) ktype("mbtsls") weight_var(wpop90)
	local est_mbtsls_se = string(_se[t], "%9.3f")

	bootstrap t=r(delta), rep(400): overid_chao, z(s_ind_? s_ind_??) x(Dlwage) y(DOmega_w_pmlall_noT) weight_var(wpop90)
	local est_hful_se = string(_se[t], "%9.3f")
restore	
...
foreach var of varlist `ind_stub'* {
	...
	qui regress `x' `temp' [aw=`weight'], robust
	local pi_`ind' = _b[`temp']
	qui test `temp'
	local F_`ind' = r(F)
	qui regress `y' `temp' [aw=`weight'], robust
	local gamma_`ind' = _b[`temp']
	drop `temp'
	}

foreach var of varlist `ind_stub'6 `ind_stub'7 `ind_stub'11 `ind_stub'13 `ind_stub'9 {
	if regexm("`var'", "`ind_stub'(.*)") {
		local ind = regexs(1) 
		}
	tempvar temp
	qui gen `temp' = `var' * `growth_stub'`ind'
	ch_weak, p(.05) beta_range(-30(.1)30)   y(`y') x(`x') z(`temp') weight(`weight')
	disp r(beta_min) ,  r(beta_max)
	local ci_min_`ind' =string( r(beta_min), "%9.1f")
	local ci_max_`ind' = string( r(beta_max), "%9.1f")
...
bartik_weight, z(`ind_stub'*) x(`x') weightstub(`growth_stub'*) y(`y') weight_var(`weight')
...
file open fh  using "./results/bartik/rotemberg_summary.tex", write replace
...
graph export "./results/bartik/noFEs_overid.png", replace
...
graph export "./results/bartik/noFEs_F_vs_rotemberg_weight.png", replace
...
foreach ind in 6 7 11 13 9 {
	if `ci_min_`ind'' != -30 & `ci_max_`ind'' != 30 {
...
file write fh  " & $\alpha$-weighted 	& Share of 			& Mean  \\ " _n
